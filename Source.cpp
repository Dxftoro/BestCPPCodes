...
#pragma warning(disable: 4018) //как же мен€ бесит это предупреждение, уберу его...

using namespace std;

string path_format(string p) { //функци€ форматировани€ указанного пути к файлу
	string result;
...
string transliter(string orig_t) { //функци€ выполн€юща€ саму "латинизацию"
	string kwords = "јЅ¬√ƒ≈®∆«»… ЋћЌќѕ–—“”‘’÷„Ўў№џЏЁёяабвгдеЄжзийклмнопрстуфхцчшщьыъэю€,-:;.!?&%$#*@^[]{}()+=0123456789 ";
	string lwords[99] = {
		"A", "B", "V", "G",
		"D", "E", "Yo", "Zh",
		"Z", "I", "Y", "K",
	...
	for (int i = 0; i < orig_t.size(); i++) {
		bool found = false; //переменна€ дл€ проверки на наличие символа в алфавите программы
		...
		for (int k = 0; k < kwords.size(); k++) {
			if (orig_t[i] == kwords[k]) { //если символ из текста совпадает с символом из кириллического алфавита
				...
				found = true; //фиксируем то, что символ совпал с одним из тех, которые наход€тс€ в алфавите
			}
		}

		if (found == false) tran_t = tran_t + orig_t[i]; //если символ так и не был найден, добавл€ем его без перевода
		found = false;
	}
...
int main() { //мейн... -_-
	...
	ofstream fout("translated.txt"); //поток вывода переведЄнного текста в файл

	string path; //переменна€ пути к файлу
	string orig_w, orig_t, tran_t = "";

	cout << "Input file path: " << endl;
	getline(cin, path); //ввод пути к файлу

	path = path_format(path); //форматируем введЄнный путь
	fin.open(path); //открываем файл с текстом
	if (!fin) { //Ќ≈ существует ли указанный файл?
		cout << "Error: Can't open specified file!" << endl;
		return 2;
	}

	fin.seekg(0); //устанавливаем курсор в самом начале текстового файла
	while (!fin.eof()) { //цикл считывани€ текста из файла
		fin >> orig_w;
	...
	//закрываем файлы в об€зательном пор€дке!
	fin.close();
	fout.close();
